...
#include <wchar.h>		//MG
#include "Fl_Native_File_Chooser_common.cpp"		// strnew/strfree/strapp/chrcat
...
// GET FILENAME
char* Fl_Native_File_Chooser::filename() const {
	if ( _pathnames && _tpathnames > 0 ) return(_pathnames[0]);
	return("");
}

// GET FILENAME FROM LIST OF FILENAMES
char* Fl_Native_File_Chooser::filename(int i) const {
